...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
    ...
    //_Ui->controllerPadGraphics->viewport()->grabGesture(Qt::TapGesture, Qt::ReceivePartialGestures);
    //_Ui->controllerPadGraphics->viewport()->grabGesture(Qt::TapAndHoldGesture, Qt::ReceivePartialGestures);
    //_Ui->controllerPadGraphics->viewport()->grabGesture(Qt::SwipeGesture, Qt::ReceivePartialGestures);
        ...
        if (event->type() == QEvent::Gesture) {
            ...
            if (QGesture *gesture = gesture_event->gesture(Qt::PinchGesture)) {
                ...
                if (change_flags & QPinchGesture::RotationAngleChanged) {
                    ...
                    //qDebug() << (pinch->rotationAngle() - pinch->lastRotationAngle()) << d << _Pad.velocity_scale_omega;
                }
...
void MainWindow::connectToNodes(const QString &namespace_name) {
    ...
    if (!namespace_name.isEmpty()) {
        ...
        _Subscribers.motion = _NodeThread->node()->create_subscription<phoenix_msgs::msg::StreamDataMotion>(
            motion_topic_name.toStdString(), qos, [this](const std::shared_ptr<phoenix_msgs::msg::StreamDataMotion> msg) { std::atomic_store(&_LastMessages.motion, msg); });
        ...
        _Subscribers.control = _NodeThread->node()->create_subscription<phoenix_msgs::msg::StreamDataControl>(
            control_topic_name.toStdString(), qos, [this](const std::shared_ptr<phoenix_msgs::msg::StreamDataControl> msg) { std::atomic_store(&_LastMessages.control, msg); });
