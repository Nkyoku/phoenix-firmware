...
static constexpr float WEIGHT_C = 10.0f;
...
#define SOLVE5(i0)                                                                              \
    ...
    float b[1];                                                                                 \
    b[0] = (active_set_sign & (1 << i0)) ? -current_limit(i0) : current_limit(i0);              \
    ...
    l[i0] = (active_set_sign & (1 << i0)) ? -l[i0] : l[i0]
...
#define SOLVE6(i0, i1)                                                                                           \
    ...
    float b[2];                                                                                                  \
    b[0] = (active_set_sign & (1 << i0)) ? -current_limit(i0) : current_limit(i0);                               \
    b[1] = (active_set_sign & (1 << i1)) ? -current_limit(i1) : current_limit(i1);                               \
    ...
    l[i0] = (active_set_sign & (1 << i0)) ? -l[i0] : l[i0];                                                      \
    l[i1] = (active_set_sign & (1 << i1)) ? -l[i1] : l[i1]
...
#define SOLVE7(i0, i1, i2)                                                                                                        \
    ...
    b[0] = (active_set_sign & (1 << i0)) ? -current_limit(i0) : current_limit(i0);                                                \
    b[1] = (active_set_sign & (1 << i1)) ? -current_limit(i1) : current_limit(i1);                                                \
    b[2] = (active_set_sign & (1 << i2)) ? -current_limit(i2) : current_limit(i2);                                                \
    ...
    l[i0] = (active_set_sign & (1 << i0)) ? -l[i0] : l[i0];                                                                       \
    l[i1] = (active_set_sign & (1 << i1)) ? -l[i1] : l[i1];                                                                       \
    l[i2] = (active_set_sign & (1 << i2)) ? -l[i2] : l[i2]

#define SOLVE8()                                                                                                                                  \
    constexpr auto T = mergeBlocks(Q, D.t(), D).inv().round(ROUND_BIT);                                                                           \
    float b[4];                                                                                                                                   \
    b[0] = (active_set_sign & 0x1) ? -current_limit(0) : current_limit(0);                                                                        \
    b[1] = (active_set_sign & 0x2) ? -current_limit(1) : current_limit(1);                                                                        \
    b[2] = (active_set_sign & 0x4) ? -current_limit(2) : current_limit(2);                                                                        \
    b[3] = (active_set_sign & 0x8) ? -current_limit(3) : current_limit(3);                                                                        \
    ...
    l[0] = (active_set_sign & 0x1) ? -l[0] : l[0];                                                                                                \
    l[1] = (active_set_sign & 0x2) ? -l[1] : l[1];                                                                                                \
    l[2] = (active_set_sign & 0x4) ? -l[2] : l[2];                                                                                                \
    l[3] = (active_set_sign & 0x8) ? -l[3] : l[3]

static constexpr inline float sqr(float x) {
    return x * x;
}

int AccelerationLimitter::compute(const Vector4f& accel_in, const Vector4f& current_limit, Vector4f& accel_out, Vector4f& current_out) {
    ...
    const float KC = WHEEL_RADIUS / MOTOR_TORQUE_CONSTANT;
    ...
    for (iteration = 0; iteration < 10; iteration++) {
        ...
        if ((over_current < 1e-6f) || (norm2 < 1e-12f)) {
            ...
            if (min_lower_bound_t < min_upper_bound_t) {
                ...
                active_set_sign |= 1 << min_lower_bound_index;
                ...
                active_set_sign &= ~(1 << min_upper_bound_index);
